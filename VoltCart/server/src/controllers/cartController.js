...
import { releaseReservationIfCartChanged } from '../services/inventoryService.js';
...
/** Any cart edit can invalidate an open checkout hold, so reconcile first. */
async function respondAfterChange(res, cart, userId) {
  await releaseReservationIfCartChanged(userId);
...
export const setCartItem = asyncHandler(async (req, res) => {
  ...
  await cart.save();
  await respondAfterChange(res, cart, req.user._id);
});
