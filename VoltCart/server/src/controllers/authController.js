...
const registerSchema = z.object({
  ...
  role: z.nativeEnum(ROLES).default(ROLES.BUYER),
...
export const register = asyncHandler(async (req, res) => {
  ...
  // Admins never shop, so there is no guest cart to carry over.
  if (data.role === ROLES.BUYER) await mergeGuestCart(user._id, guestItems);
  issueToken(res, user);
...
export const login = asyncHandler(async (req, res) => {
  ...
  if (user.role === ROLES.BUYER) await mergeGuestCart(user._id, guestItems);
  issueToken(res, user);
