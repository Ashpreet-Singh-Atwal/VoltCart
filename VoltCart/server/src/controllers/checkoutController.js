...
import { getReservation, releaseReservation, reserveStock } from '../services/inventoryService.js';

const userReservationKey = (userId) => `fs:user-resv:${userId}`;

async function readActiveReservation(userId) {
...
export const createReservation = asyncHandler(async (req, res) => {
  ...
  if (existing && sameItems(existing.items, saleLines.map((l) => ({ productId: l.productId, quantity: l.quantity })))) {
    return res.json({
      reservation: {
