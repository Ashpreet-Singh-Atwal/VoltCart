...
import {
  ACTIVE_SALE_KEY, metaKey, reservedKey, saleNs, soldKey, stateKey, stockKey,
} from '../redis/keys.js';
...
/** Deletes every Redis key belonging to one sale. */
export async function clearSaleRedis(saleId) {
  const pattern = `${saleNs(String(saleId))}:*`;
  const keys = [];
  let cursor = '0';
  do {
...
/**
 * Rewrites the Redis counters of a running sale after an admin edit. Units
 * already sold or held stay untouched; only the free pool is recalculated.
 */
export async function syncLiveSaleStock(sale) {
  const saleId = sale._id.toString();

  for (const item of sale.items) {
    const productId = item.product.toString();
    const [sold, reserved] = await redis.mget(soldKey(saleId, productId), reservedKey(saleId, productId));
    const taken = Number(sold ?? 0) + Number(reserved ?? 0);
    const free = item.totalQuantity - taken;
    if (free < 0) {
      throw new Error(`${taken} units of this deal are already sold or held; quantity cannot drop below that.`);
    }
    await redis.set(stockKey(saleId, productId), free);
    await redis.hset(metaKey(saleId, productId), {
      salePrice: item.salePrice,
      originalPrice: item.originalPrice,
      totalQuantity: item.totalQuantity,
      maxPerUser: item.maxPerUser,
  ...
  await redis.hset(stateKey(saleId), {
    status: sale.computedStatus(),
    startAt: new Date(sale.startAt).getTime(),
    endAt: new Date(sale.endAt).getTime(),
  });
